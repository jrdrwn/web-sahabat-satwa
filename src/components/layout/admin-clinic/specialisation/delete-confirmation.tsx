...
export default function DeleteConfirmationButton({
  ...
  async function onSubmit(data: Specialisation[]) {
    const resAll = async (specialisation: Specialisation) => {
      const res = await fetch(
        `/api/admin-clinic/specialisation/${specialisation.spec_id}`,
        {
          method: 'DELETE',
          headers: {
    ...
    if (results.every((result) => result)) {
      ...
      toast.warning('Sebagian spesialisasi gagal dihapus');
      router.refresh();
    } else {
      ...
      <ResponsiveModal open={open} onOpenChange={handleOpenChange}>
        ...
        <ResponsiveModalContent>
          <ResponsiveModalHeader className="mb-4">
            <ResponsiveModalTitle>
              Apakah Anda yakin ingin menghapus Spesialisasi ini?
            </ResponsiveModalTitle>
            <ResponsiveModalDescription>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-center">Spesialisasi</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {specialisation.map((specialisation) => (
                  <TableRow key={specialisation.spec_id}>
                    <TableCell className="text-center font-medium">
                      {specialisation.spec_id}
                    </TableCell>
          ...
          <ResponsiveModalFooter className="gap-2">
            ...
            <Button
              variant="destructive"
              onClick={() => onSubmit(specialisation)}
            >
              Delete
            </Button>
          </ResponsiveModalFooter>
