...
const formSchema = z.object({
  ...
  vetTitle: z.string().trim().max(4, 'Gelar maksimal 4 karakter').optional(),
  vetGivenname: z
    .string({
...
export default function CreateAccountForm() {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    const res = await fetch('/api/vet/register', {
      ...
      body: JSON.stringify({
        vet_title: data.vetTitle,
        vet_givenname: data.vetGivenname,
        vet_familyname: data.vetFamilyname,
        vet_phone: data.vetPhone,
        email: data.email,
